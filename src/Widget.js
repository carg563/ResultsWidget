...
function (declare, lang, array, domStyle, domClass, on, aspect, topic, domQuery, domAttr, _WidgetsInTemplateMixin, _TemplatedMixin,
    BaseWidget, WidgetManager, FeatureLayer, FeatureSet, domConstruct, PanelManager, LoadingShelter,
    ...
    return declare([BaseWidget, _WidgetsInTemplateMixin, _TemplatedMixin], {
        ...
        postCreate: function () {
            ...
            //  pm.resize({ w: 550 });
            domStyle.set(pm.domNode, { "width": "100%" });
        },
        startup: function () {
            this.inherited(arguments);
        ...
        _populateToolbar: function () {
            ...
            var reportBtn = new Button({
                showLabel: false,
                'class': 'resultswidget-tbar-btn',
                iconClass: "report-view ",
                title: "Open Report Widget",
                style: "float:right",
                onClick: lang.hitch(this, function () {
                    this._hideWidgetPanel(this.id + '_panel');
                    this._displayWidgetPanel("Report");
        ...
        onReceiveData: function (name, widgetId, data, historyData) {
            ...
            if (data.interogate && data.isReport === undefined) {
        ...
        _report: function (feature) {

            rwData = new FeatureSet();
            rwData.name = feature._layer.name;
            rwData.features = [];
            rwData.uniqueId = 1001;
            rwData.uniqueFeatureIdentifier = "APPUNIQUEID";
            rwData.serviceUrl = feature._layer.url.replace(/[/][0-9]+/, '');
            rwData.serviceLayerIntId = parseInt(feature._layer.url.substr(feature._layer.url.length - 1));
            rwData.layerIdInMap = rwData.serviceLayerIntId;
            rwData.layer = new FeatureLayer(feature._layer.url);
            rwData.customFields = [];
            rwData.fields = feature._layer.fields;
            rwData.geometryType = feature._layer.geometryType;
            feature.attributes["APPUNIQUEID"] = 1001;
            rwData.features.push(feature);

            reportWidgetName = this.config.reportWidgetName;
            var reportWidget = this.widgetManager.getWidgetsByName(reportWidgetName);
            if (reportWidget.length == 0) {
                var rw = this.appConfig.getConfigElementsByName(reportWidgetName)[0];
                if (rw) {

                    this.openWidgetById(rw.id).then(lang.hitch(this, function (widget) {
                        this._publish(this.publishData, [rwData]);
                ...
                if (reportWidget[0].state === 'opened') {
                    this._displayWidgetPanel(reportWidgetName);
                }
                if (reportWidget[0].state === 'closed') {
                    this.widgetManager.openWidget(reportWidget[0]);
                    this._displayWidgetPanel(reportWidgetName);
                    //this.panelManager.showPanel(reportWidget[0]);
                }
                this._publish(this.publishData, [rwData]);
            }
            this._hideWidgetPanel(this.id + "_panel");
        ...
        _publish: function (pub, data, symbology) {
            pub({
                interogate: true,
                results: data,
                isReport: true,
                zoomToResults: false,
                fromEaglePopup: false,
                symbology: symbology
